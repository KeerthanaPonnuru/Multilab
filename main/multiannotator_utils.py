...
def temp_scale_pred_probs(
    ...
    scaled_log_probs = np.log(pred_probs) / temp
    scaled_pred_probs = np.exp(scaled_log_probs)

    # Each label's probability is treated independently, so we normalize each probability
    # independently to ensure they are valid (i.e., between 0 and 1)
    scaled_pred_probs = scaled_pred_probs / np.sum(scaled_pred_probs, axis=1, keepdims=True)
